...
st.markdown("""
    This is a skin cancer detection system application. Upload an image, and the model will predict 
...
if uploaded_image is not None:
    ...
    # displaying the result image with the prediction result
    st.image(uploaded_image, caption="Uploaded Image", use_container_width=True)  # Show the uploaded image first
    st.write(f"**Prediction: {class_label}**")

    # # displaying processed result image with the prediction result
    # fig, ax = plt.subplots()
    # ax.imshow(img)
...
st.markdown("""
    ### About the Model:
    This model uses 2D CNN architecture for predicting whether a skin lesion 
    is **Benign** or **Malignant** based on images of skin lesions.
